...
#OUT:  Plot of  observed depth with rainfall

#' Observed and Simulated Water Levels Plot
#'
#' Create a plot of observed depth  with rainfall
#' 
...
marsWaterLevelPlot <- function(event, #group_by variable, simulated or observed event, for title block
                          df, #nested dataframe
                          #user inputs 
  ...
  if(nrow(comb_data) == 0){
    ...
    #First Re-rerun marsDetectEvents to identify end of rainfall event
    ...
    sim_plot <- 
      ...
      geom_hline(yintercept = 0, color = "black", size = 1.2)+ #top
      
      geom_hline(yintercept = max_depth_ft, color = "orange", size = 1.2)+ #top
      
      geom_label(aes(x = min_date + event_duration/4, 
                     y = max_depth_ft*1.04, 
      ...
      # annotate("text", x = min_date + event_duration/4, 
      #          y = max_depth_ft*1.04,
      ...
      #                                         override.aes = list(
      #                                           linetype = "solid", "solid")) #observed, simulated (dotted format is difficult to read - intentionally set as solid)
      # )+
    ...
    maxWidth = grid::unit.pmax(cwlGrob$widths[2:9], rainGrob$widths[2:9])
    cwlGrob$widths[2:9] <- maxWidth
    rainGrob$widths[2:9] <- maxWidth
